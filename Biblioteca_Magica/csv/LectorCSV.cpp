...
bool LectorCSV::tieneComillasValidas(const std::string& campoOriginal) {
    try {
        ...
        std::cerr << "Error en LectorCSV::tieneComillasValidas: " << e.what() << std::endl;
        return false;
    }
}
...
std::string LectorCSV::limpiarCampo(const std::string& campoOriginal) {
    try {
        ...
        std::cerr << "Error en LectorCSV::limpiarCampo: " << e.what() << std::endl;
        return "";
    }
}
...
//sobrecarga al constructor para que reciba la ruta
void LectorCSV::procesarArchivo(const std::string &ruta) {
    try {
        rutaArchivo = ruta;
        procesarArchivo();
    } catch (const std::exception& e) {
        std::cerr << "Error en LectorCSV::procesarArchivo (sobrecarga): " << e.what() << std::endl;
        throw;
    }
}
...
void LectorCSV::procesarArchivo() {
    try {
        ...
        std::cerr << "Error en LectorCSV::procesarArchivo: " << e.what() << std::endl;
        log("Error crítico durante el procesamiento del archivo: " + std::string(e.what()));
    }
}
...
bool LectorCSV::agregarLibroIndividual(const std::string& titulo, const std::string& isbn, const std::string& genero, const std::string& fecha, const std::string& autor) {
    try {
        ...
        std::cerr << "Error en LectorCSV::agregarLibroIndividual: " << e.what() << std::endl;
...
bool LectorCSV::validarISBN(const std::string& isbn) {
    try {
        ...
        std::cerr << "Error en LectorCSV::validarISBN: " << e.what() << std::endl;
        return false;
    }
}
